...
public class JDay extends Model implements MongoModel {
	...
	public static List<JDay> addAbsenceDay(final Map<DateTime, F.Tuple<Boolean, Boolean>> dates, final ObjectId userId, final ObjectId missionId, final String comment) {
		...
		for(DateTime dt : dates.keySet()) {
			if(cra == null || (cra.year != dt.getYear() || cra.month != dt.getMonthOfYear())) {
				cra = JCra.getOrCreate(userId, dt.getYear(), dt.getMonthOfYear());
			}
			// Search JDay...
			...
			if(dates.get(dt)._1.equals(Boolean.TRUE)) {
				day.morning = new JHalfDay(missionId);
			}
			if(dates.get(dt)._2.equals(Boolean.TRUE)) {
				day.afternoon = new JHalfDay(missionId);
			}
	...
	public static void deleteAbsenceDays(final Map<DateTime, F.Tuple<Boolean, Boolean>> dates, final ObjectId userId) {

		Set<F.Tuple<Integer, Integer>> yearMonth = Sets.newHashSet();
		for(DateTime dt : dates.keySet()) {
	...
	public static List<JDay> find(final String userId, final Set<DateTime> dateTimes) {
		return q()
			.field("userId").equal(userId)
