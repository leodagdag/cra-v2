...
public class JDay extends Model implements MongoModel {
    ...
    public static void addAbsenceDay(final JAbsence absence) {
        ...
        JCra cra = null;
        for (DateTime dt : dts) {
            final DateTime curr = dt.withTimeAtStartOfDay();
            if (cra == null || (cra.year != curr.getYear() || cra.month != curr.getMonthOfYear())) {
                cra = JCra.getOrCreate(absence.userId, curr.getYear(), curr.getMonthOfYear());
            }
            // Search JDay...
            JDay day = JDay.find(cra.id, curr);
            ...
            if (curr.isEqual(absence.startDate.withTimeAtStartOfDay())) {
                // First Day
                if (absence.startDate.getHourOfDay() == 0) {
                    day.morning = new JHalfDay(absence.missionId);
                }
                day.afternoon = new JHalfDay(absence.missionId);
            } else if (curr.isEqual(absence.endDate.withTimeAtStartOfDay())) {
                // Last Day
                day.morning = new JHalfDay(absence.missionId);
                if (absence.endDate.getHourOfDay() == 0) {
                    day.afternoon = new JHalfDay(absence.missionId);
                }
            } else {
                day.morning = new JHalfDay(absence.missionId);
